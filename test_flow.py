...
def main(opt):
    ...
    dl = torch.utils.data.DataLoader(ds, batch_size=cfg.TRAIN.IMS_PER_BATCH, shuffle=True,
                                     drop_last=False, collate_fn=ds.collate_fn_, num_workers=cfg.DATA_LOADER.NUM_THREADS)
    ...
    # checkpoint 로딩 (flow 래퍼 파라미터가 저장된 경우/SDT만 저장된 경우 모두 허용)
    ...
    with torch.no_grad():
        for _ in tqdm.tqdm(range(total_batches)):
            ...
            pred = flow.flow_infer(img_list, char_img, T=T, steps=opt.steps,
                                   stride=opt.stride, replan=opt.replan)   # [B,T,5]
